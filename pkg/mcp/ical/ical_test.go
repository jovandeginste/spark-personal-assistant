...
END:VCALENDAR`
	...
	cal := Calendar{Name: "Test Calendar", URL: ts.URL}
	events, err := m.searchEvents(cal, []string{"birthday"}, "", "")
	require.NoError(t, err)
	require.Len(t, events, 1)
	assert.Equal(t, "My Birthday", events[0].Summary)

	// 3. Test description match
	events, err = m.searchEvents(cal, []string{"delivery"}, "", "")
	require.NoError(t, err)
	require.Len(t, events, 1)
	...
	events, err = m.searchEvents(cal, []string{"Day"}, "2026-01-01", "2026-01-31")
	require.NoError(t, err)
	require.Len(t, events, 1)
	...
	events, err = m.searchEvents(cal, []string{"Year"}, "2026-02-01", "2026-12-31")
	require.NoError(t, err)
	...
	// 5. Test no matches
	events, err = m.searchEvents(cal, []string{"NonexistentEvent"}, "", "")
	require.NoError(t, err)
	assert.Empty(t, events)
	...
	events, err = m.searchEvents(calWithMeta, []string{"Official"}, "", "")
	require.NoError(t, err)
	require.Len(t, events, 3)

	// "Work" appears in calendar name
	events, err = m.searchEvents(calWithMeta, []string{"Work"}, "", "")
	require.NoError(t, err)
	require.Len(t, events, 3)
}
